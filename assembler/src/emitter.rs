...
impl Emittable {
        ...
        match &self.instruction {
            Instruction {
                ...
                match operands.as_slice() {
                    ...
                    _ => panic!(
                        "Only one string operand is .STRINGZ in {:?}",
                        self.instruction
                    ),
                }
            }
    ...
    pub fn clamp(&self, value: i64, n_bits: u8) -> Result<i64, String> {
        ...
        if value < lower_limit || value > upper_limit {
            Err(format!(
                "Value '{}' was not within valid bounds: [{}, {}]",
    ...
    pub fn unsupported_operands_err<T>(&self) -> Result<T, String> {
        return Err(format!(
            ...
            self.instruction.opcode, self.instruction.operands
        ));
    }
    ...
    pub fn emit(&self, state: &Lc3State) -> Result<Vec<u16>, String> {
        match &self.instruction {
            Instruction {
                opcode: Opcode::Ld | Opcode::Ldi | Opcode::Lea,
                operands,
            } => {
            ...
            Instruction {
                ...
                match operands.as_slice() {
                    [Operand::Label { name }] => {
                        let offset = state
                            .relative_offset(self.offset, name, 11)
            ...
            Instruction {
                ...
                if operands.len() > 0 {
                    return Err(format!(
                        "TRAP alias '{:?}' does not take any parameters",
            ...
            Instruction {
                ...
                match operands.as_slice() {
                    [Operand::Label { name }] => {
                        ...
                        if imm9_opt.is_none() {
                            return Err(format!(
                                "Label used for BR instruction not found: {}",
                                name
                            )
                            .into());
                        }
            ...
            Instruction {
                ...
                let res = operands
                    ...
                    .map(|x| match x {
                        ...
                        Operand::Label { name } => match state.labels.get(name) {
                            Some(x) => x.to_owned() as u16,
                            _ => panic!("Did not find label with name '{:?}' in .FILL", name),
                        },
                        _ => panic!(
                            "Only immediate operands are allowed for fill in {:?}",
                            self.instruction
                        ),
                    })
                    .collect();
            ...
            Instruction {
                ...
                operands.iter().for_each(|x| match x {
                    Operand::String { value } => {
                    ...
                    _ => panic!(
                        "Only string operands are allowed for .STRINGZ in {:?}",
                        self.instruction
                    ),
                });
